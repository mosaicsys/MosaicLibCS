...
namespace MosaicLib.WPF.Converters
{
	...
	public class LogGateStringConverter: IValueConverter
	{
		public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
		{
            ...
            {
                if (value is MosaicLib.Logging.LogGate)
                    return ((MosaicLib.Logging.LogGate)value).ToString(allowTerseVersion: true);
            }
            else if (targetType == typeof(MosaicLib.Logging.LogGate))
            {
                MosaicLib.Logging.LogGate logGate = MosaicLib.Logging.LogGate.All;
                if (logGate.TryParse(vc.GetValue<string>(rethrow: false).MapNullToEmpty()))
                    return logGate;
            }
